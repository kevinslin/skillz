...
- allow safe overwrite checks for copied skill directories,
...
### Use cache during target validation

- `src/commands/sync.ts` + `src/core/skill-target-manager.ts`

```text
cachedSkillPaths = keys(cache.skills) or empty set
validateSkillTargets(..., cachedSkillPaths)
...
This allows previously copied skill directories to be overwritten safely.

**File(s)**: `src/commands/sync.ts`, `src/core/skill-target-manager.ts`
...
flowchart TD
  ...
  K -- "no" --> M["validate target directories (uses cached skill keys)"]
  M --> N["copy skills to target directories"]
